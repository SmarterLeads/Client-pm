-- Ensure report_slug exists and Overview inline edits persist via update_client_with_team_member_context.

ALTER TABLE public.clients
  ADD COLUMN IF NOT EXISTS report_slug text;

CREATE UNIQUE INDEX IF NOT EXISTS idx_clients_report_slug_unique
  ON public.clients (report_slug)
...
BEGIN
  ...
  SET
    ...
    google_drive_url = CASE
      WHEN p_payload ? 'google_drive_url' THEN NULLIF(p_payload->>'google_drive_url', '')
    ...
    report_slug = CASE
      WHEN p_payload ? 'report_slug' THEN NULLIF(TRIM(p_payload->>'report_slug'), '')
      ELSE report_slug
    END,
    show_in_dashboard = CASE
      WHEN p_payload ? 'show_in_dashboard' THEN COALESCE(
        (p_payload->>'show_in_dashboard')::boolean,
        false
      )
      ELSE show_in_dashboard
    END,
    whatconverts_profile_id = CASE
      WHEN p_payload ? 'whatconverts_profile_id' THEN NULLIF(p_payload->>'whatconverts_profile_id', '')
      ELSE whatconverts_profile_id
    END,
