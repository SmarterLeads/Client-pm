-- Marketing dashboard fields on client create/update (report slug, dashboard toggle, WhatConverts).
...
BEGIN
  ...
  SET
    ...
    report_slug = CASE
      WHEN p_payload ? 'report_slug' THEN NULLIF(p_payload->>'report_slug', '')
      ELSE report_slug
    END,
    show_in_dashboard = CASE
      WHEN p_payload ? 'show_in_dashboard' THEN COALESCE((p_payload->>'show_in_dashboard')::boolean, false)
