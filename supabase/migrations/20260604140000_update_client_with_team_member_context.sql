...
BEGIN
  ...
  SET
    ...
    email = CASE
      WHEN p_payload ? 'email' THEN NULLIF(p_payload->>'email', '')
      ELSE email
    END,
    phone = CASE
      WHEN p_payload ? 'phone' THEN NULLIF(p_payload->>'phone', '')
      ELSE phone
    END,
    company = CASE
      WHEN p_payload ? 'company' THEN NULLIF(p_payload->>'company', '')
      ELSE company
    END,
    status = CASE
      WHEN p_payload ? 'status' THEN (p_payload->>'status')::public.client_status
      ELSE status
    END,
    rag_status = CASE
      WHEN p_payload ? 'rag_status' THEN (p_payload->>'rag_status')::public.rag_status
      ELSE rag_status
    END,
