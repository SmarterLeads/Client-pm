...
export function QuickAddTaskForm({
  ...
  useEffect(() => {
    ...
    setExpanded(false);
    router.refresh();
    onCreatedRef.current?.(state.taskId);
  ...
  return (
    <form action={formAction} className="space-y-2 rounded-lg border border-border bg-card p-2">
      <input type="hidden" name="_quick_add" value="true" />
