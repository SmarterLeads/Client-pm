...
} from "@/lib/marketing/report/st-james-report-config";
import {
  isBreakdownShown,
...
type ConversionBreakdownCardRow = {
  id: string;
  label: string;
  conversionType: string;
  current: number;
...
function breakdownDisplayRowKey(c: BreakdownCfg, clientId: string): string {
  if (
  ...
  return displayKey ? `display:${displayKey}` : `raw:${c.platform}:${c.raw_name}`;
}

function breakdownDisplayRowLabel(c: BreakdownCfg, rowKey: string): string {
  if (rowKey === ST_JAMES_COMBINED_PURCHASE_MERGE_BUCKET) {
    return ST_JAMES_COMBINED_PURCHASE_LABEL;
  }
  return conversionDisplayLabel(c);
}

async function fetchMaxiMindConversionBreakdownCards(
  supabase: SB,
  clientId: string,
  windows: DateWindow,
  platformSlug: string | null,
  opts?: { breakdownBooleans?: ChannelMetricMap },
): Promise<ConversionBreakdownCardRow[]> {
  const platformForBuild = platformSlug?.trim() || null;

  let configQuery = supabase
    .from("client_conversions")
    .select(
      "id, raw_name, platform, display_name, mapped_name, sort_order, conversion_type, is_active",
    )
    .eq("client_id", clientId)
    .eq("is_active", true)
    .order("sort_order", { ascending: true });
  if (platformForBuild) {
    configQuery = configQuery.ilike("platform", platformForBuild);
  }
  const { data: activeConfigs, error: configErr } = await configQuery;
  if (configErr) throw configErr;

  const rawNames = getMaxiMindRawNamesForQuery(activeConfigs ?? [], platformForBuild);
  console.log("[MaxiMind] querying raw_names:", rawNames);
  if (rawNames.length === 0) return [];

  const breakdownBooleans = opts?.breakdownBooleans;
  const allowedRawNames = breakdownBooleans
    ? rawNames.filter((raw) => isBreakdownShown(breakdownBooleans, raw))
    : rawNames;
  if (allowedRawNames.length === 0) return [];

  const [{ data: eventData, error: eventErr }, { data: convData, error: convErr }] =
    await Promise.all([
      supabase
        .from("conversion_events")
        .select("occurred_on, event_name, platform, event_count")
        .eq("client_id", clientId)
        .in("event_name", allowedRawNames)
        .gte("occurred_on", windows.priorStart)
        .lte("occurred_on", windows.currentEnd),
      supabase
  ...
  console.log("[MaxiMind] events found:", eventData);

  const totalsCurrent = new Map<string, number>();
  const totalsPrior = new Map<string, number>();
  const allowedSet = new Set(allowedRawNames);

  for (const row of eventData ?? []) {
    if (platformForBuild && !platformSlugMatchesRow(row.platform ?? "", platformForBuild)) {
      continue;
    }
    const eventName = (row.event_name ?? "").trim();
    if (!eventName || !allowedSet.has(eventName)) continue;
    const n = Number(row.event_count ?? 0);
    if (row.occurred_on >= windows.currentStart) {
      totalsCurrent.set(eventName, (totalsCurrent.get(eventName) ?? 0) + n);
    } else if (row.occurred_on >= windows.priorStart && row.occurred_on <= windows.priorEnd) {
      totalsPrior.set(eventName, (totalsPrior.get(eventName) ?? 0) + n);
    }
  }
  ...
  for (const row of convData ?? []) {
    ...
    if (row.report_date >= windows.currentStart) {
      totalsCurrent.set(eventName, (totalsCurrent.get(eventName) ?? 0) + convCount);
    } else if (row.report_date >= windows.priorStart && row.report_date <= windows.priorEnd) {
      totalsPrior.set(eventName, (totalsPrior.get(eventName) ?? 0) + convCount);
    }
  }

  const useMetaAggKeys =
    platformForBuild?.toLowerCase() === "meta" ||
    (!platformForBuild &&
      (eventData ?? []).some((row) => platformSlugMatchesRow(row.platform ?? "", "meta")));

  const groups = buildMaxiMindConversionBreakdown({
  ...
  return (groups[0]?.rows ?? []).map((row) => ({
    id: row.id,
    label: row.displayName,
    conversionType: row.type,
    current: row.totalCount,
    prior: row.priorCount,
  }));
}
...
export async function fetchConversionBreakdownCards(
  ...
  if (clientId === MAXI_MIND_CLIENT_ID) {
    return fetchMaxiMindConversionBreakdownCards(
      supabase,
      clientId,
      windows,
      platformSlug,
  ...
  const eligibleConfigs = allConfigs ?? [];
  ...
  const sorted = [...cfgs].sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));
  ...
  const displayMap = new Map<string, DisplayGroupAgg>();

  for (const c of sorted) {
    const pairKey = activeConversionPairKey(c.platform ?? "", c.raw_name);
    const rowKey = breakdownDisplayRowKey(c, clientId);
    if (!rowKey) continue;

    const label = breakdownDisplayRowLabel(c, rowKey);
    const sortOrder = c.sort_order ?? 999999;

    const prev = displayMap.get(rowKey);
    if (!prev) {
      displayMap.set(rowKey, {
        rowKey,
        id: c.id,
        label,
    ...
    prev.sortOrder = Math.min(prev.sortOrder, sortOrder);

    const bestSortOrder = prev.bestConfig.sort_order ?? 999999;
    if (sortOrder <= bestSortOrder) {
      prev.id = c.id;
      prev.label = label;
      prev.conversionType = c.conversion_type;
      prev.bestConfig = c;
    }
  }

  const displayGroups = Array.from(displayMap.values()).sort((a, b) => {
    if (isEcommerceClient) {
  ...
  const topGroups = displayGroups.filter((g) => {
    let cur = 0;
    let prior = 0;
    for (const pk of Array.from(g.pairKeys)) {
    ...
    const isPurchaseBucket =
      (g.bestConfig.conversion_type ?? "").trim().toLowerCase() === "purchase";
    const isStJamesPurchaseBucket = g.rowKey === ST_JAMES_COMBINED_PURCHASE_MERGE_BUCKET;
